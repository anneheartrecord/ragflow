...
// fallback="" and stores nothing for an invalid_at the LLM made up, so the
// memory stays valid. The Go side must not turn that same output into
// "expired at extraction time" by falling back to now — the two
// implementations would give one memory two different lifetimes.
